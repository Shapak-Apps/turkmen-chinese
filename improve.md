...
| Измерение | Старт | Цель | Статус (обновлено 2026-06-07) |
|---|---|---|---|
...
- Облачный бэкап (Supabase) — пока локального достаточно.
- Туркменизация контента — фаза команды переводчиков после готовности.

**⚠️ Нужно проверить на устройстве (device-QA):**
reward-гард (повтор без XP) · shadowing-произношение (нет микрофона, есть «Dowam et») · пуш-напоминание (промпт + 19:00) · масштаб шрифта · доступность · локальный бэкап (export/import).
