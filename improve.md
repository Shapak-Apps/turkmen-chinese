...
- [~] 🟠 **M — Аналитика.** ✅ Каркас `lib/analytics.ts` (facade + таксономия) + инструментация воронки (app_open, onboarding, chapter/theory open, lesson start/complete) + тесты.
  - [ ] подключить провайдера (PostHog cloud реком.) — **нужен твой API-ключ**, потом ~10 строк адаптера + init.
...
- [x] 🟠 **M — Пуш-напоминания о streak** ✅ Локальное ежедневное напоминание (19:00) через `expo-notifications` + переключатель в Sazlamalar (`remindersEnabled`, по умолч. вкл). Без сервера. ⚠️ требует device-QA (промпт прав + доставка).
